...
public class Day15Ex2 implements ExSolution {
    ...
    public static void main(String[] args) {
        ResolveApplication.main(new String[]{"15", "2"});
    }
...
//    }
    ...
    private void sequence(int left, HashMap<Integer, Integer> recette, int index) {
        if (index == 3 || left == 0) {
            recette.put(index, left);
            result = Math.max(result, calculate(recette));
            return;
        }
        for (int n = 0; n <= left; n++) {
            recette.put(index, n);
            sequence(left - n, recette, index + 1);
        }
    }

    private long calculate(HashMap<Integer, Integer> recette) {
        Long total = 1L;
        for (int i = 0; i < 4; i++) {
            Long property = 0L;
            for (int j = 0; j < array.length; j++) {
                property += array[j][i] * recette.getOrDefault(j, 0);
            }
        ...
        //System.out.println(recette + " = " + total);

        // calcul calories
        long calories = 0L;
        for (int i = 0; i < recette.size(); i++) {
            calories += array[i][4] * recette.getOrDefault(i, 0);
        }
        if(calories != 500){
            return 0;
        }
